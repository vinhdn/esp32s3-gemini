...
static void car_exec_cb(void *var, int32_t v)
{
    ...
    lv_obj_align((lv_obj_t *)var, LV_ALIGN_TOP_MID, off * 3 / 50, 84);
}
...
static void build_lane(void)
{
    ...
    /* three vertical tracks: left lane, right lane, faint centre line.
     * Each track is 2x screen height and scrolls by one dash period. */
    const int lane_x[3] = { 37, 132, 84 };
    const int lane_w[3] = { 2, 2, 1 };
    const int dash_h[3] = { 14, 14, 8 };
    const int gap_h[3]  = { 16, 16, 18 };

    for (int i = 0; i < 3; i++) {
        lane_track[i] = plain(lane_panel);
        lv_obj_set_size(lane_track[i], lane_w[i], HUD_SCR_H * 2);
        lv_obj_set_pos(lane_track[i], lane_x[i], 0);
        int period = dash_h[i] + gap_h[i];
        for (int y = 0; y < HUD_SCR_H * 2; y += period) {
            lv_obj_t *d = plain(lane_track[i]);
            lv_obj_set_size(d, lane_w[i], dash_h[i]);
            lv_obj_set_pos(d, 0, y);
            lv_obj_set_style_bg_color(d, i == 2 ? HUD_C_LINE : HUD_C_LANE, 0);
            lv_obj_set_style_bg_opa(d, LV_OPA_COVER, 0);
        }
    }

    /* the car, top-down, cyan like the mockup */
    car = plain(lane_panel);
    lv_obj_set_size(car, 40, 64);
    lv_obj_align(car, LV_ALIGN_TOP_MID, 0, 84);
    lv_obj_set_style_radius(car, 9, 0);
    lv_obj_set_style_bg_color(car, HUD_C_CYAN, 0);
    lv_obj_set_style_bg_grad_color(car, HUD_C_CYAN_DARK, 0);
    lv_obj_set_style_bg_grad_dir(car, LV_GRAD_DIR_VER, 0);
    lv_obj_set_style_bg_opa(car, LV_OPA_COVER, 0);

    for (int i = 0; i < 2; i++) {          /* windscreen + rear window */
        lv_obj_t *g = plain(car);
        lv_obj_set_size(g, 28, i ? 11 : 13);
        lv_obj_align(g, i ? LV_ALIGN_BOTTOM_MID : LV_ALIGN_TOP_MID, 0, i ? -8 : 9);
        lv_obj_set_style_radius(g, 4, 0);
        lv_obj_set_style_bg_color(g, lv_color_hex(0x04222A), 0);
    ...
    lv_anim_set_values(&lane_anim, -30, 0);
